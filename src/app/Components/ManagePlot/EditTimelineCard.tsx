...
export default function EditTimelineCard({ card, onCancel }: EditCardProps) {
  ...
  // initialize form states with card data
  const [eventTag, setEventTag] = useState<eventsType>(card.event_tag);
  const [cardTitle, setCardTitle] = useState(card.card_title);
  const [cardColor, setCardColor] = useState(card.card_color);
  const [chapter, setChapter] = useState(card.chapter);
  const [details, setDetails] = useState<detailsType[]>(card.details || []);

  useEffect(() => {
    if (showSuccessNotification) {
      const timer = setTimeout(() => {
        setShowSuccessNotification(false);
        onCancel(); // close edit mode after success
      }, 2000);
      return () => clearTimeout(timer);
    }
  }, [showSuccessNotification, onCancel]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await updateTimelineCard(formPayload, token);
      if (res.ok) {
  ...
  return (
    <div className='relative flex items-start ml-16'>
      ...
      <div
        ...
        className={`absolute -left-8 w-4 h-4 rounded-full border-4 border-white shadow-lg cursor-pointer`}
