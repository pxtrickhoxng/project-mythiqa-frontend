...
import {
  fetchTimelineCards,
  fetchLatestTimelineIndex,
} from "@/lib/api/timeline";
import { fetchBookCover } from "@/lib/api/books";
        ...
        <div className="flex justify-between items-start mb-8">
          <div className="flex-1">
            ...
            <p className="text-gray-600">
              Organize your plot structure and key story events
            </p>
          </div>
          {coverUrl && (
        ...
        <div className="relative">
          <div className="space-y-12">
            <AddTimelineCard
              bookId={bookId}
              userId={userId}
              latestIndex={data.latest_timeline_index}
            />

            {timelineEvents && timelineEvents.length > 0 ? (
              timelineEvents.map((note) => (
                <TimelineCardDisplay key={note.index} note={note} />
              ))
            ) : (
